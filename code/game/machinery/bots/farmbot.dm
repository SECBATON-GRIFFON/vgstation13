...
/obj/machinery/bot/farmbot/proc/use_farmbot_item()
	...
	if (!emagged && !istype(target,/obj/machinery/portable_atmospherics/hydroponics) && !istype(target,/obj/machinery/atmospherics/unary/wc/sink) ) // Humans are not plants!
		mode = 0
		target = null
		return
...
/obj/machinery/bot/farmbot/target_selection()
	if (emagged) //Find a human and help them!
		...
		if(setting_refill && tank && tank.reagents.total_volume < 500 )
			for(var/obj/machinery/atmospherics/unary/wc/sink/source in view(7,src) )
				target = source
...
/obj/machinery/bot/farmbot/proc/refill()
	if ( !tank || !tank.reagents.total_volume > 600 || !istype(target,/obj/machinery/atmospherics/unary/wc/sink) )
		mode = 0
		target = null
		return
