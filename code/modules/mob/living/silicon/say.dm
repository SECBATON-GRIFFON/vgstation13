...
/mob/living/proc/robot_talk(var/message, var/emote = FALSE)


	var/turf/T = get_turf(src)
	log_say("[key_name(src)] (@[T.x],[T.y],[T.z] Binary: [message]")

	var/message_a = emote ? html_encode(message) : say_quote("\"[html_encode(message)]\"")
	...
	for (var/mob/S in player_list)
		if(istype(S , /mob/living/silicon/ai))
			var/renderedAI = "<i><span class='binaryradio'>Robotic Talk, <a href='byond://?src=\ref[S];track=[name]'><span class='name'>[name]</span></a> <span class='message'>[message_a]</span></span></i>"
