...
/var/const/access_mint = 51 //not in use
/var/const/access_mint_vault = 52 //not in use
...
/var/const/access_shop = 68 //not in use, but a valid option for access for custom airlocks/etc
	...
	if(!A)
		...
		if(user && (A.loc == user))
			if(A.icon_state == "door_electronics_smoked")
				to_chat(user, "<span class='warning'>Repair \the [A] before putting it in!</span>")
				return
...
/proc/get_all_accesses()
	return list(access_shop, access_security, access_sec_doors, access_brig, access_armory, access_forensics_lockers, access_court,
	            ...
	            access_keycard_auth, access_tcomsat, access_gateway, access_paramedic, access_mechanic, access_biohazard)
...
/proc/get_region_accesses(var/code)
	switch(code)
		...
		if(7) //supply
			return list(access_mailsorting, access_mining, access_mining_station, access_cargo, access_cargo_bot, access_qm)
