...
/obj/item/weapon/virusdish/afterattack(var/atom/A, var/mob/user, var/adjacency_flag, var/click_params)
	...
	if (open)
		...
		if (istype(A,/obj/machinery/atmospherics/unary/wc/toilet))
			var/obj/machinery/atmospherics/unary/wc/toilet/T = A
			if (T.open)
				empty(user,A)
		if (istype(A,/obj/machinery/atmospherics/unary/wc/urinal)||istype(A,/obj/machinery/atmospherics/unary/wc/sink))
			empty(user,A)
