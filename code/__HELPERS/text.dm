...
/proc/feedize(list/proposed, filename, cullshort = 1, storemax = 1000, culllong = 280)
	if(!islist(proposed) || !filename || !config.log_feed)
		return
	...
	for(var/string in proposed)
		if(findtext(string,is_website) || findtext(string,is_email) || findtext(string,all_invalid_symbols) || !findtext(string,is_alphanumeric))
			continue
		var/buffer = ""
		...
		for(var/pos = 1, pos <= lentext, pos += length(let))
			let = string[pos]
			if(!findtext(let, is_alphanumeric))
				continue
			early_culling = FALSE
		...
		for(var/pos in 1 to lentext)
			...
			if(!findtext(let, is_punctuation)) //This won't handle things like Nyaaaa!~ but that's fine
				break
			punctbuffer += let
			cutoff += length(let)
		...
		lentext = length_char(buffer)
		if(!buffer || lentext > culllong || lentext <= cullshort || (buffer in accepted))
			continue
